...
class AudioGenerator():
    def __init__(self, mfcc_dim=13, minibatch_size=16, desc_file=None, 
    ...
    def shuffle_data_by_partition(self, partition):
        """ Shuffle the train or valid data
        """
        if partition == 'train':
    ...
    def sort_data_by_duration(self, partition):
        """ Sort the train or valid sets by (increasing) duration
        """
        if partition == 'train':
    ...
    def fit_train(self, k_samples=500):
        """ Estimate the mean and std of the features from the training set
        Params:
            k_samples (int): number of samples
        """
...
def vis_train_features(index=0):
    """ Visualize a data in training set by index.
    """
    ...
    return vis_text, vis_raw_audio, vis_mfcc_feature, vis_audio_path
